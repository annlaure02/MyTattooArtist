  ...
  const handleSearch = async (data) => {
    ...
    try {
      ...
      if (searchTerm && searchStyleTerm) {
        url += `search=${searchTerm}&style=${searchStyleTerm}`;
      } else if (searchTerm) {
        url += `search=${searchTerm}`;
      } else if (searchStyleTerm) {
        url += `style=${searchStyleTerm}`;
      }
      console.log('searchStyleTerm:', searchStyleTerm);
        ...
        <section className='search--icon'>
          <form className='search__form' onSubmit={handleSubmit(handleSearch)} >
            <input className='search-input'
              ...
              {...register('searchTerm')}
            />
            <select className='search-input-style'
              placeholder='styles'
              {...register('searchStyleTerm')}
            >
              <option >Styles</option>
              {styles.map(style => (
                <option key={style.id} value={style.style_name}>{style.style_name}</option>
              ))}
            </select>
